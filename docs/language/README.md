...
3. [Compiler optimizations](compiler-optimizations.md) — sync fast paths (`eq-unify`, …), `--disable-opt`, and TLA+ `--disable-tla-opt` (`unused-fields`, `unused-vars`, `determined-args`, `from-collection`, `literal-domains`, `unwrap-singletons`, `unused-lets`)
